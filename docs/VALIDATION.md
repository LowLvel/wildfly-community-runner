...
8. Artifact watcher hardening (in progress): content fingerprints, stable ZIP output, changes during in-flight deployment, directory recreation, source filtering, expired generations, cross-project suppression and deduplication, and serialized scanner targets. Filesystem tests emulate scanner acknowledgements with a fake managed process; they are not a WildFly runtime test.
