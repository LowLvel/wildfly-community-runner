...
sample workspace in the IntelliJ test application's default Swing theme. Its 1280×800
PNG output is retained as the `marketplace-screenshots` CI artifact. These component
screenshots show the actual controls, selection, and status renderers.
The sample log and scanner markers are fixture data; real-server behavior is
covered separately by `WildFlyRuntimeTest`.

Reviewed images are stored in [services.png](images/services.png) and
[server-log.png](images/server-log.png), generated by the successful Linux build in
[CI run 35769099713](https://github.com/LowLvel/wildfly-community-runner/actions/runs/35769099713).
Review regenerated images before replacing
these files or uploading them to Marketplace Media. Keep the same aspect ratio, avoid personal paths and real
